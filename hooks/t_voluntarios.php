<?php
	...
	function t_voluntarios_init(&$options, $memberInfo, &$args)
	{
...
//echo '<BR><BR><BR><BR>'.$perm[2].'<BR>'.$DisplayRecords.'<BR>'.$Filtro.'<BR>InfoUsuario[0]:'.$memberInfo['custom'][0].'<BR>InfoUsuario[1]:'.$memberInfo['custom'][1];
		...
		{ 
			$options->QueryWhere=" where `t_voluntarios`.`IdPPAM` = '" .$memberInfo['custom'][0]."' ";
		}
		...
		{ 
			if ($memberInfo['custom'][1] != 99)
			{
				...
				$options->QueryWhere=" where `t_ubicaciones_ppam`.`IdZonaCoordinacion` = '" .$memberInfo['custom'][1] ."' and `t_voluntarios`.`IdPPAM` = `t_ubicaciones_ppam`.`IdPPAM` "; 
			}
		}
	...
	function t_voluntarios_header($contentType, $memberInfo, &$args){
		$header='';
	...
	function t_voluntarios_footer($contentType, $memberInfo, &$args){
		$footer='';
	...
	function t_voluntarios_before_insert(&$data, $memberInfo, &$args){

		return TRUE;
	}

	function t_voluntarios_after_insert($data, $memberInfo, &$args){

		return TRUE;
	}

	function t_voluntarios_before_update(&$data, $memberInfo, &$args){

		return TRUE;
	}

	function t_voluntarios_after_update($data, $memberInfo, &$args){

		return TRUE;
	}

	function t_voluntarios_before_delete($selectedID, &$skipChecks, $memberInfo, &$args){

		return TRUE;
	}

	function t_voluntarios_after_delete($selectedID, $memberInfo, &$args){

	}

	function t_voluntarios_dv($selectedID, $memberInfo, &$html, &$args){

	}

	function t_voluntarios_csv($query, $memberInfo, &$args){

		return $query;
	}
	function t_voluntarios_batch_actions(&$args){
